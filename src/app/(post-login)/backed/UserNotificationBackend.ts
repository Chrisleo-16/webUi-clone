...
class UserNotificationBackend {
    ...
    async markNotificationUserAsRead(notification: NotificationModel): Promise<{ success: boolean; message: string }> {
        try {
            await this.notificationService.markNotificationAsUserRead(notification.id);
            return {
                success: true,
                message: "Notification marked as read",
            };
...
export default new UserNotificationBackend(new SystemLogService());
