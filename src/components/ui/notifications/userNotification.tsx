...
import UserNotificationBackend from "@/app/(post-login)/backed/UserNotificationBackend";
...
const UserNotificationMenu = () => {
  // Fetch user notifications from the Redux store
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        await BackedAdminSystemReport.fetchAndDispatchNotificationsUser();
      } catch (error) {
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    dispatch(markUserNotificationAsRead(notification.id));
  ...
  const markNotificationAsRead = async (notification: Notification) => {
    const { success, message } =
      await UserNotificationBackend.markNotificationUserAsRead(notification);
    if (!success) {
  ...
  const markAllNotificationAsRead = async () => {
    const { success, message } =
      await UserNotificationBackend.markAllNotificationsAsRead();
    if (!success) {
      showToast(message, "error");
    } else {
      showToast(message, "success");
